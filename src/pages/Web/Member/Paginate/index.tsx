...
import Editor from '@/pages/Web/Member/Editor';
...
const Paginate: React.FC = () => {
  ...
  const [search, setSearch] = useState<APIWebMembers.Search>({});
  const [editor, setEditor] = useState<APIWebMembers.Data | undefined>();
  const [visible, setVisible] = useState<APIWebMembers.Visible>({});
  const [data, setData] = useState<APIData.Paginate<APIWebMembers.Data>>();

  const toPaginate = () => {
    setLoad(true);
    doPaginate(search)
      .then((response: APIResponse.Paginate<APIWebMembers.Data>) => {
  ...
  const onDelete = (record: APIWebMembers.Data) => {
    if (data?.data) {
      const temp = {...data};
      if (temp.data) {
  ...
  const onEnable = (record: APIWebMembers.Data) => {
    if (data?.data) {
      const temp = {...data};
      if (temp.data) {
  ...
  const onUpdate = (record: APIWebMembers.Data) => {
    setEditor(record);
    setVisible({...visible, editor: true});
  };
      ...
      >
        ...
        >
          ...
          <Table.Column
            title="职位"
            align="center"
            render={(record: APIWebMembers.Data) => (
              <Tag color={initialState?.settings?.colorPrimary}>{record.title}</Tag>
            )}
          />
          <Table.Column
            title="序号"
