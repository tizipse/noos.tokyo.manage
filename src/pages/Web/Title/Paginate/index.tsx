...
import Editor from '@/pages/Web/Title/Editor';
...
const Paginate: React.FC = () => {
  ...
  const [search, setSearch] = useState<APIWebTitles.Search>();
  const [editor, setEditor] = useState<APIWebTitles.Data | undefined>();
  const [load, setLoad] = useState(false);
  const [visible, setVisible] = useState<APIWebTitles.Visible>({});
  const [data, setData] = useState<APIData.Paginate<APIWebTitles.Data>>();
  ...
  const onDelete = (record: APIWebTitles.Data) => {
  ...
  const onEnable = (record: APIWebTitles.Data) => {
    if (data) {
      const temp = {...data};
      if (temp.data) {
  ...
  const onUpdate = (record: APIWebTitles.Data) => {
    setEditor(record);
    setVisible({...visible, editor: true});
  };
      ...
      <Card
        ...
        extra={
          <Space size={[10, 10]} wrap>
            ...
            <Access accessible={access.page('web.title.create')}>
              <Tooltip title="创建">
      ...
      >
        ...
        >
          ...
          <Table.Column
            title="序号"
            align="center"
            render={(record: APIWebTitles.Data) => (
          ...
          <Table.Column
            ...
            align="center"
            render={(record: APIWebTitles.Data) => (
              <Access accessible={access.page('web.title.enable')}
          ...
          <Table.Column
            ...
            render={(record: APIWebTitles.Data) => (
              <>
                <Access accessible={access.page('web.title.update')}>
                  <Button type="link" onClick={() => onUpdate(record)}>
                    编辑
                  </Button>
                </Access>
                <Access accessible={access.page('web.title.delete')}>
                  <Popconfirm
                    title="确定要删除该数据?"
                    placement="leftTop"
